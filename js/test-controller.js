...
class TestController {
    constructor() {
        ...
        this.level = null;
        this.correctChinese = null;   // 当前单词正确选项的中文（意→中）
        this.currentOptionChinese = []; // 四个选项的中文，与 currentOptions 一一对应，保证形式一致
    ...
    async generateOptions(wordData) {
        ...
        try {
            ...
            // 四个选项都显示「英文 (中文)」：正确选项用意→中，错误选项用英→中
            this.correctChinese = await translationHelper.getChineseTranslation(wordData.word);
            const distractorChinese = await Promise.all(
                distractors.map(d => translationHelper.getChineseFromEnglish(d))
            );
            const allChinese = [this.correctChinese, ...distractorChinese];

            // 选项与中文一一对应，一起打乱顺序
            const pairs = allOptions.map((opt, i) => [opt, allChinese[i] || null]);
            this.shuffleArray(pairs);
            this.currentOptions = pairs.map(p => p[0]);
            this.currentOptionChinese = pairs.map(p => p[1]);
    ...
    renderOptions() {
        ...
        this.currentOptions.forEach((option, index) => {
            ...
            const chinese = this.currentOptionChinese[index];
            button.textContent = chinese ? `${option} (${chinese})` : option;
    ...
    async selectOption(selectedIndex) {
        ...
        const historyEntry = {
            ...
            options: [...this.currentOptionsOrder],
            optionChinese: [...(this.currentOptionChinese || [])], // 四个选项的中文，用于「上一个」恢复
            correctChinese: this.correctChinese,
    ...
    async showPreviousWord(wordInfo) {
        ...
        this.currentOptionChinese = Array.isArray(wordInfo.optionChinese) ? [...wordInfo.optionChinese] : [];
        ...
        const optionChinese = this.currentOptionChinese || [];
        this.currentOptions.forEach((option, index) => {
            const button = document.createElement('button');
            button.className = 'option-button';
            const chinese = optionChinese[index];
            button.textContent = chinese ? `${option} (${chinese})` : option;
            if (index === this.correctAnswerIndex) button.classList.add('correct');
            if (index === wordInfo.selectedIndex && index !== this.correctAnswerIndex) button.classList.add('wrong');
