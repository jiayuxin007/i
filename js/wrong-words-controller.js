...
class WrongWordsController {
    ...
    async init() {
        ...
        // 更新页面标题
        document.getElementById('pageTitle').textContent = `${this.level} 错题本`;
    ...
    // 加载错题本单词
    loadWrongWords() {
        const wrongWordList = storageManager.getWrongWords(this.level);
        
        // 从所有单词中筛选出错题本中的单词
        this.wrongWords = this.allWords.filter(word => wrongWordList.includes(word.word));
        
        console.log(`${this.level} 错题本共有 ${this.wrongWords.length} 个单词`);
    }
    ...
    loadPosFilter() {
        ...
        const posList = Array.from(posSet).sort();
    ...
    async displayWords() {
        ...
        if (this.filteredWords.length === 0) {
            container.innerHTML = '<div class="no-results">错题本为空，或没有找到匹配的单词</div>';
        ...
        pageWords.forEach(word => {
            ...
            tableHTML += `
                <tr>
                    <td>
                        <a href="word-detail.html?word=${encodeURIComponent(word.word)}&level=${this.level}&returnTo=wrong-words" 
                    ...
                    <td>
                        <button class="remove-button" onclick="wrongWordsController.removeWord('${word.word}')">
                            移除错题本
                        </button>
                    </td>
                </tr>
    ...
    async removeWord(word) {
        if (confirm(`确定要将 "${word}" 从错题本中移除吗？`)) {
            await storageManager.removeWrongWord(this.level, word);
            console.log(`已将单词 "${word}" 从错题本移除`);
    ...
    displayPagination(totalPages) {
        ...
        // 上一页按钮
        paginationHTML += `
            <button class="page-button" onclick="wrongWordsController.goToPage(${this.currentPage - 1})" 
        ...
        if (startPage > 1) {
            paginationHTML += `<button class="page-button" onclick="wrongWordsController.goToPage(1)">1</button>`;
            if (startPage > 2) {
        ...
        for (let i = startPage; i <= endPage; i++) {
            paginationHTML += `
                <button class="page-button ${i === this.currentPage ? 'active' : ''}" 
                        onclick="wrongWordsController.goToPage(${i})">
                    ${i}
                </button>
            `;
        }
        ...
        if (endPage < totalPages) {
            ...
            paginationHTML += `<button class="page-button" onclick="wrongWordsController.goToPage(${totalPages})">${totalPages}</button>`;
        }
        ...
        paginationHTML += `
            ...
            <input type="number" class="page-input" id="pageInput" min="1" max="${totalPages}" 
                   value="${this.currentPage}" onkeypress="if(event.key==='Enter') wrongWordsController.jumpToPage()">
...
// 创建全局实例
const wrongWordsController = new WrongWordsController();
...
function applyFilters() {
    wrongWordsController.applyFilters();
}

// 页面加载时初始化
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => wrongWordsController.init());
} else {
    wrongWordsController.init();
}
